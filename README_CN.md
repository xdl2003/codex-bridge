...
- 定时把设定好的 prompt 自动发送给恢复后的 Codex 会话
...
- **Codex 定时任务**  
  可以给某个 workspace 会话创建周期任务或一次性任务。任务到时间后，本地 server 会自动把设定的 prompt 发给 Codex。
...
3. **定时任务**
   - 默认把任务定义保存到 `~/.codex-bridge/jobs.json`
   - 运行中的 server 进程会检查到期任务，并通过同一条恢复会话执行路径发送 prompt
...
```bash
node ./bin/codex-bridge.js schedule <session-id> "检查当前 git 状态" \
  --workspace /path/to/project \
  --every 60
```
...
定时任务只保存在运行 Codex Bridge 的本机。一个任务包含：
...
只有 Codex Bridge server 进程运行时，任务才会自动执行。如果希望任务按时触发，需要保持 `npm run server` 或 `codex-bridge server` 处于运行状态。

任务 prompt 可以使用普通聊天一样的 `$skill-name` mention。网页创建任务时会提取已知 skill id 并传给后端。
...
```json
{
  ...
  "prompt": "$plot-from-data 汇总最新运行结果",
